-- Шаг 1: Считаем Recency, Frequency, Monetary для каждого клиента
WITH base_rfm AS (
    SELECT
        customer_id,
...
-- Шаг 2: Вычисляем Recency как разницу между 2024-06-01 и последней датой заказа
rfm_with_recency AS (
    SELECT
        customer_id,
...
-- Шаг 3: Присваиваем баллы R, F, M на основе квантилей
scored_rfm AS (
    SELECT
        customer_id,
        recency,
...
-- Шаг 4: Присваиваем RFM-код и сегмент
final_rfm AS (
    SELECT
        customer_id,
